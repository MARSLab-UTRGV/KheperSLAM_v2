...
CKheperaIVForaging::SFoodData::SFoodData() :
   HasFoodItem(false),
   FoodItemIdx(0),
   TotalFoodItems(0) {}

void CKheperaIVForaging::SFoodData::Reset() {
   HasFoodItem = false;
   FoodItemIdx = 0;
   TotalFoodItems = 0;
}
...
CKheperaIVForaging::SDiffusionParams::SDiffusionParams() :
   GoStraightAngleRange(CRadians(-1.0f), CRadians(1.0f)) {}

void CKheperaIVForaging::SDiffusionParams::Init(TConfigurationNode& t_node) {
   try {
...
void CKheperaIVForaging::SWheelTurningParams::Init(TConfigurationNode& t_node) {
   try {
...
CKheperaIVForaging::SStateData::SStateData() :
   ProbRange(0.0f, 1.0f) {}

void CKheperaIVForaging::SStateData::Init(TConfigurationNode& t_node) {
   try {
...
void CKheperaIVForaging::SStateData::Reset() {
   State = STATE_RESTING;
   InNest = true;
...
CKheperaIVForaging::CKheperaIVForaging() :
   m_pcWheels(NULL),
   m_pcLEDs(NULL),
   m_pcRABA(NULL),
   m_pcRABS(NULL),
...
void CKheperaIVForaging::Init(TConfigurationNode& t_node) {
   try {
      /*
      ...
      m_pcGround    = GetSensor  <CCI_KheperaIVMotorGroundSensor    >("kheperaiv_motor_ground" );
      /*
      ...
      THROW_ARGOSEXCEPTION_NESTED("Error initializing the kheperaiv foraging controller for robot \"" << GetId() << "\"", ex);
   }
   /*
...
void CKheperaIVForaging::ControlStep() {
...
void CKheperaIVForaging::UpdateState() {
   /* Reset state flags */
   m_sStateData.InNest = false;
   /* Read stuff from the ground sensor */
   const CCI_KheperaIVMotorGroundSensor::TReadings& tGroundReads = m_pcGround->GetReadings();
   /*
...
CVector2 CKheperaIVForaging::CalculateVectorToLight() {
...
CVector2 CKheperaIVForaging::DiffusionVector(bool& b_collision) {
...
void CKheperaIVForaging::SetWheelSpeedsFromVector(const CVector2& c_heading) {
...
void CKheperaIVForaging::Explore() {
...
void CKheperaIVForaging::ReturnToNest() {
 ...
 */
REGISTER_CONTROLLER(CKheperaIVForaging, "kheper_foraging_controller")
